...
class AlbScreen(Screen):
    ...
    def compose(self) -> ComposeResult:
        with Horizontal(classes="screen-layout"):
            ...
            with VerticalScroll(classes="form-container"):
                ...
                yield Label(
                    "CloudFront custom domain (optional):", classes="section-label"
                )
                yield Input(
                    placeholder="cdn.example.com", id="cloudfront_custom_domain"
                )
                yield Label(
                ...
                yield Label(
                    "Query allowlist (comma separated):", classes="section-label"
                )
                yield Input(
                ...
                yield Label(
                    "Cookie allowlist (comma separated):", classes="section-label"
                )
                yield Input(
    ...
    def _restore_cloudfront_certificate_options(self) -> None:
        ...
        source_options = (
            draft_options if isinstance(draft_options, list) else state_options
        )
    ...
    def _validate_cloudfront(self) -> bool:
        ...
        origin_https_only = self.query_one(
            "#cloudfront_origin_https_only", Switch
        ).value
        if origin_https_only:
            ...
            if mode == "shared":
                listener_protocol = (
                    str(self._state.get("shared_listener_protocol") or "")
                    .strip()
                    .upper()
                )
                ...
                if listener_protocol != "HTTPS" or listener_port != 443:
                    listener_arn = str(
                        self._state.get("shared_listener_arn") or ""
                    ).strip()
                    if listener_arn and (
        ...
        for behavior in self._cloudfront_cached_behaviors:
            ...
            if name in behavior_names:
                self.notify(
                    f"Duplicate CloudFront behavior name '{name}'", severity="error"
                )
                return False
    ...
    def on_list_view_selected(self, event: ListView.Selected) -> None:
        ...
        if event.list_view.id == "cloudfront-behavior-list":
            ...
            self.query_one(
                "#cloudfront_behavior_query_mode", Select
            ).value = behavior.get("query_strings", "all")
            self.query_one(
                "#cloudfront_behavior_query_allowlist", Input
            ).value = ", ".join(behavior.get("query_string_allowlist", []))
            self.query_one(
                "#cloudfront_behavior_cookie_mode", Select
            ).value = behavior.get("cookies", "none")
            self.query_one(
                "#cloudfront_behavior_cookie_allowlist", Input
            ).value = ", ".join(behavior.get("cookie_allowlist", []))
            self.query_one("#cloudfront_behavior_min_ttl", Input).value = str(
    ...
    def _add_cloudfront_connection(self) -> None:
        ...
        if self._editing_cloudfront_conn_index is not None:
            self._cloudfront_connections[self._editing_cloudfront_conn_index] = (
                connection
            )
            self._editing_cloudfront_conn_index = None
        else:
    ...
    def _add_cloudfront_behavior(self) -> None:
        ...
        query_mode_value = self.query_one(
            "#cloudfront_behavior_query_mode", Select
        ).value
        query_mode = (
        ...
        if self._editing_cloudfront_behavior_index is not None:
            self._cloudfront_cached_behaviors[
                self._editing_cloudfront_behavior_index
            ] = behavior
            self._editing_cloudfront_behavior_index = None
        else:
    ...
    def _start_fetch_cloudfront_certificates(self) -> None:
        ...
        self.notify(
            "Fetching ACM certificates from us-east-1...", severity="information"
        )
        threading.Thread(
